...
- **Codex Key 校验（Right.Codes）**
  - 后端：新增 `POST /api/responses/codex/keys/validate`，使用渠道 `baseUrl` 构建 `/v1/responses`（支持 `#` 跳过版本前缀）做真实校验
  - 前端：编辑渠道添加密钥时先校验，成功才允许加入；渠道编排 Key 列表新增“检测”按钮并提示成功/失败（含 statusCode + 上游错误摘要）

### 🧪 测试

- 补充：Codex Key 校验 handler 单测覆盖（成功/403/2xx但错误封装）

### 🧩 仓库

- **版本号更新**：`VERSION` → `v2.5.7`
