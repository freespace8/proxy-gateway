...
func tryChannelWithAllKeys(
	...
	for sortedIdx, urlResult := range sortedURLResults {
		...
		for attempt := 0; attempt < maxRetries; attempt++ {
			...
			if err != nil {
				failedKeys[apiKey] = true
				common.RecordFailureAndStoreLastFailureLog(circuitLogStore, metricsManager, "responses", currentBaseURL, apiKey, 0, nil, err, func() {
...
func handleSingleChannel(
	...
	if len(enabledKeys) == 0 {
		...
		c.JSON(503, gin.H{
			"error": fmt.Sprintf("当前 Responses 渠道 \"%s\" 未配置可用API密钥", upstream.Name),
			"code":  "NO_API_KEYS",
		})
		return
	}
