...
export const useChannelStore = defineStore('channel', () => {
  ...
  const isValidApiTab = (value: unknown): value is ApiTab => {
    return value === 'messages' || value === 'responses' || value === 'gemini'
  }

  const getActiveTabFromRoute = (): ApiTab | null => {
    const route = router.currentRoute.value

    if (route.path.startsWith('/channels/')) {
      const type = route.params.type
      return isValidApiTab(type) ? type : null
    }

    if (route.path === '/monitor') {
      const type = route.query.type
      if (Array.isArray(type)) return isValidApiTab(type[0]) ? type[0] : null
      return isValidApiTab(type) ? type : null
    }
  ...
  // 监听路由变化：/channels/:type 与 /monitor?type= 同步 activeTab
  watch(
    () => [router.currentRoute.value.path, router.currentRoute.value.params.type, router.currentRoute.value.query.type],
    () => {
      const type = getActiveTabFromRoute()
  ...
  const normalizeHealth = (status: unknown): Channel['health'] | undefined => {
  ...
  async function pingChannel(channelId: number) {
    ...
    if (channel) {
      ...
      channel.health = normalizeHealth(result.status)
    }
